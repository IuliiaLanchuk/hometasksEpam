...
async def get_current_dollar_value(session) -> float:
    today = datetime.date.today().strftime("%d/%m/%Y")
    central_bank_url = f"http://www.cbr.ru/scripts/XML_daily.asp?date_req={today}"
    dollar_value = (
        BeautifulSoup(await get_html_content(session, central_bank_url), "lxml")
...
def get_value_with_reg_exp(parameter, soup) -> Optional[float]:
    pattern = r"([0-9]*\.[0-9]*)\r\n.*" + parameter
    search_value = re.search(pattern, str(soup))
...
async def get_html_content(session, url_: str) -> str:
    async with session.get(url_) as response:
        return await response.text()


async def fetch_company_name_href_and_growth(url: str, session) -> List[Dict]:
...
async def get_company_full_info(
    session, company_name_href_growth, current_dollar_value
) -> Dict:
...
def save_data_in_json(sorted_data: List[Dict], description: str, key: str):
    ...
    with open(json_file_name, "w") as file:
        json.dump(data_to_json_file, file, indent=4)
